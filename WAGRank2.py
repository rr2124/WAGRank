"""
Python implementation of WAGRank 2.
"""
import os
import numpy as np
...
def attention_score(all_sent, lexicon, topn):
    ...
    norm_fact = 1 / math.sqrt(embedding_dimension)
    attention_score_matrix = np.zeros((len(lexicon), len(all_sent)), dtype='float')
    for m in range(len(all_sent)):
        s = all_sent[m]
        s_vector = 0
        lex_size = len(s)
        if lex_size > 0:
            for h in range(lex_size):
                s_vector += embedding_dic[s[h]] / lex_size
            for word in s:
                attention_score_matrix[vocab[word]][m] = np.dot(embedding_dic[word], s_vector) * norm_fact
    attention_score_dic = np.sum(attention_score_matrix, axis=1)
...
def create_graph(lexicon, textlist_filtered_write, topn):
    ...
    for word in lexicon:
        ...
        for w in topsim:
            board[vocab[w]][vocab[word]] = max(0, attention_score_dic[vocab[w]] + attention_score_dic[vocab[word]])
            board[vocab[word]][vocab[w]] = board[vocab[w]][vocab[word]]
    return board
